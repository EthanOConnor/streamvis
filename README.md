...
- `c` toggles the chart metric between stage and flow; `b` toggles the bell/flash alert when new data is fetched; `r` requests an immediate refresh; `f` forces an immediate refetch even if the last observation time has not advanced; `q` quits.
...
- Add `?community=...` to the page URL to set `--community-base` (cached in browser localStorage as `streamvis_community_base`).
- Add `&publish=1` to also enable `--community-publish` (cached as `streamvis_community_publish`).
- Your community endpoint must serve CORS headers (e.g., `Access-Control-Allow-Origin: *`) for the browser to POST samples.
